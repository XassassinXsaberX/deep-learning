...
# 接下來要建立"字典"
token = Tokenizer(num_words=3800) # 建立一個有3800字的字典
...
# 接下來使用sequence.pad_sequences() 方法將數字list截長補短，使所有數字list的長度都為380
x_train = sequence.pad_sequences(x_train_seq, maxlen=380) # 為一個有25000個元素的ndarray，每個元素是一個list，該list的元素則存放著轉換後的數字 (現在強制變為380個)
x_test = sequence.pad_sequences(x_test_seq, maxlen=380) # 為一個有25000個元素的ndarray，每個元素是一個list，該list的元素則存放著轉換後的數字 (現在強制變為380個)
...
model.add(LSTM(32)) # 建立"LSTM 層"，有32個神經元
...
try:
    model.load_weights("./save_model/IMDB_LSTM.h5")
    print("成功載入模型，繼續訓練模型")
except:
    print("載入模型失敗，開始訓練模型")
...
# 再來把模型儲存起來
model.save_weights("./save_model/IMDB_LSTM.h5")
